--- src/services/listoneService.ts (原始)
...
function normalizeRole(role: string): Role {
  ...
  if (r === 'P' || r === 'POR' || r === 'PORTIERE') return 'P';
  if (r === 'D' || r === 'DIF' || r === 'DIFENSORE') return 'D';
  if (r === 'C' || r === 'CEN' || r === 'CENTROCAMPISTA') return 'C';
  if (r === 'A' || r === 'ATT' || r === 'ATTACCANTE') return 'A';
  return 'C';
}
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const roleCol = findColumn(headers, ['ruolo', 'role', 'ruolo classic', 'r']);
        const qiCol = findColumn(headers, ['quotazione iniziale', 'qi', 'quotazione', 'prezzo', 'value']);
...
+++ src/services/listoneService.ts (修改后)
import { Player, Role } from '../types';
import { allPlayers as fallbackPlayers, serieATeams } from '../data/players';
import * as XLSX from 'xlsx';
