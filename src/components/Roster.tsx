  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (ext === 'json') {
        ...
        const headerRowIndex = json.findIndex(row => row.some(cell => typeof cell === 'string' && cell.toLowerCase().includes('calciatore') || cell.toLowerCase().includes('nome')));
        
        if (headerRowIndex === -1) throw new Error('Colonna "Calciatore" o "Nome" non trovata nel file.');
        ...
        setImportError('Formato non supportato. Usa .xlsx, .csv o .json');
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-slate-900 to-emerald-950 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-6 relative">
          <button onClick={onBack} className="absolute left-0 top-0 text-slate-400 hover:text-white transition-colors">
            ← Indietro
          </button>
        ...
        {/* 1. PANNELLO IMPORTAZIONE INTELLIGENTE (Foto/Excel) */}
        <AIImport
          availablePlayers={availablePlayers}
          currentRoster={localRoster}
          onPlayersFound={(newPlayers) => {
            setLocalRoster(prev => [...prev, ...newPlayers]);
          }}
        />
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <div className="text-sm text-slate-400">
                  {listoneStatus?.fileName ? `📄 ${listoneStatus.fileName} • ${listoneStatus.playerCount} giocatori` : `${listoneStatus?.playerCount || 0} giocatori caricati`}
                </div>
              </div>
            </div>
        ...
        {/* 3. Barra di Ricerca (con FIX z-50) */}
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-700 border border-slate-600 rounded-xl shadow-xl z-[9999] max-h-96 overflow-y-auto">
                {searchResults.map(player => (
                  <button key={player.id} onClick={() => addPlayer(player)} className="w-full px-4 py-3 text-left hover:bg-slate-600/50 transition-colors flex items-center justify-between border-b border-slate-600/50 last:border-0">
                    ...
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${player.role === 'P' ? 'bg-yellow-500/20 text-yellow-400' : player.role === 'D' ? 'bg-blue-500/20 text-blue-400' : player.role === 'C' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {player.role}
                    </span>
                  </button>
                ))}
                      ...
                      {players.map(player => (
                        <div key={player.id} className="flex items-center justify-between p-3 bg-slate-700/40 rounded-lg hover:bg-slate-700/60 transition-colors">
                          <div>
                            ...
                            <div className="text-slate-400 text-xs">{player.team} • FM: {player.fantamedia} • Tit: {player.titolarita}% • Avv: {player.avversario}</div>
                          </div>
        ...
        <div className="mt-6 bg-slate-800/60 backdrop-blur-sm rounded-xl border border-emerald-500/20 p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="text-slate-300">
              {isRosterComplete ? <span className="text-emerald-400 font-medium">✓ Rosa completa (25 giocatori)</span> : hasMinimumPlayers ? <span className="text-amber-400 font-medium">⚠ Rosa parziale - puoi procedere</span> : <span className="text-red-400 font-medium">✗ Aggiungi almeno 11 giocatori</span>}
            </div>
